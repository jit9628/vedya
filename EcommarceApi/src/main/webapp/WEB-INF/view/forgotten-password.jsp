...
<body>
	...
	<div class="container d-none verifierotp">
		<div class="row my-5">
			<div class="col-md-4 offset-md-4 p-5 login">
				<h2>Otp Verification</h2>
				<form class="mt-4"  method="get">
					
					<div class="mb-3" id="d3">
						<label class="form-label">Enter Otp</label> <input type="text"
							class="form-control" placeholder="Enter Otp" 
							id="Otpverify" name="otpverifier">
					</div>
					<!-- <div class="mb-3 form-check">
                            <input type="checkbox" class="form-check-input" >
                            <label class="form-check-label">Check me out</label>
                        </div> -->
					<button type="button" class="btn mb-3 w-100 " id="otp-verifier">Otp Verification</button>
					<p><a href="/api/page/SignUp">Login Up</a>
					</p>
				</form>
			</div>
...
$("#forgatepassword").click(function(){
	 $.ajax({
		...
		$("#forgatepassword").html("Verify Otp");
		
		//$('#d1').hide();
		//$('#d2').hide();
		//$(".main-form").hide();
		//$(".verifierotp").removeClass("d-none");
		},
...
/* -------------------Otp Verification--------------- */
$("#otp-verifier").click(function(){
	var pass=$("#password").val();
	  $.ajax({
		type:'post',
		url:'/api/test/verifiy-otp',
		dataType: 'json',
		contentType: "application/json",
		headers: {
			'Accept': 'application/json',
